...
def train_sweeps(model, loader, optimizer, mlm_criterion, cls_criterion, epoch, device, logger, wandb_logger=None, \
                use_thresholds=True, diag_freeze=False, mlm_lambda=0.5, num_classes=100, mlm_loss_type='ce', seed=123):
    # 모델을 학습하기 위한 함수
    model.train()
    train_loss = 0.0
    train_mlm_loss = 0.0
    ...
    logger.info(f'[Epoch train {seed}-{epoch}]: Total loss: {tr_avg_loss:.4f} | MLM loss: {tr_avg_mlm_loss:.4f} | CLS loss: {tr_avg_cls_loss:.4f}')
    logger.info(f'[Epoch train {seed}-{epoch}]: Acuuracy: {acc:.4f}, AUC: {auc:.4f}, F1: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}')
    wandb_logger.log(data={f'train/Total Loss-{str(seed)}': tr_avg_loss, 
                           f'train/MLM Loss-{str(seed)}': tr_avg_mlm_loss, 
                           f'train/CLS Loss-{str(seed)}': tr_avg_cls_loss,
                           'epoch': epoch})
    wandb_logger.log(data={f'train/Accuracy-{str(seed)}/': acc, 
                           f'train/AUC-{str(seed)}': auc, 
                           f'train/F1-{str(seed)}': f1, 
                           f'train/Precision-{str(seed)}': precision, 
                           f'train/Recall-{str(seed)}': recall,
                           'epoch': epoch})
    return {
            'loss': tr_avg_loss, 
            'mlm_loss': tr_avg_mlm_loss, 
...
@torch.no_grad()
def evaluate_sweeps(model, loader, mlm_criterion, cls_criterion, epoch, device, logger, wandb_logger=None,\
                    use_thresholds=True, mode='valid', mlm_lambda=0.5, num_classes=100, mlm_loss_type='ce', seed=123):
    model.eval()
    val_loss = 0.0 
    val_mlm_loss = 0.0
    val_cls_loss = 0.0
    ...
    logger.info(f'[Epoch {mode} {seed}-{epoch}]: Total loss: {val_avg_loss:.4f} | MLM loss: {val_avg_mlm_loss:.4f} | CLS loss: {val_avg_cls_loss:.4f}')
    logger.info(f'[Epoch {mode} {seed}-{epoch}]: Acuuracy: {acc:.4f}, AUC: {auc:.4f}, F1: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}')
    if mode == 'valid':
        wandb_logger.log(data={f'{mode}/Total Loss-{str(seed)}': val_avg_loss, 
                               f'{mode}/MLM Loss-{str(seed)}': val_avg_mlm_loss, 
                               f'{mode}/CLS Loss-{str(seed)}': val_avg_cls_loss,
                               'epoch': epoch})
        wandb_logger.log(data={f'{mode}/Accuracy-{str(seed)}': acc,
                               f'{mode}/AUC-{str(seed)}': auc, 
                               f'{mode}/F1-{str(seed)}': f1,
                               f'{mode}/Precision-{str(seed)}': precision, 
                               f'{mode}/Recall-{str(seed)}': recall,
                               'epoch': epoch})
    else:
        wandb_logger.log(data={f'{mode}/Accuracy-{str(seed)}': acc, 
                               f'{mode}/AUC{str(seed)}': auc, 
