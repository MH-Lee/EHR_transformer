...
class EncoderNew(nn.Module):
    def __init__(self,
        ...
        if pretrained_emb is None:
            self.pre_embedding = Embedding(vocab_size, model_dim, padding_idx=0)
        else:
            self.pretrained_emb = pretrained_emb[:, :model_dim]
            diag_size = self.pretrained_emb.size(0)
            self.pre_embedding = Embedding(vocab_size, model_dim, padding_idx=0)
            self.pre_embedding.weight.data[0:diag_size] = self.pretrained_emb
    ...
    def forward(self, diagnosis_codes, mask_code, seq_time_step, input_len):
