    ...
    with wandb.init() as writer:
        ...
        if use_pretrained:
            if wandb.config.diag_freeze:            
                wandb_name = f'EHR-Transformer_{str(seed)}_diag_freeze_GPT4O_SWEEPS'
                tags = (str(seed), 'diag_freeze', wandb.config.loss_type, wandb.config.pretrained_type)
            else:
                wandb_name = f'EHR-Transformer_{str(seed)}_GPT4O_SWEEPS'
                tags = (str(seed), wandb.config.loss_type,  wandb.config.pretrained_type)
        else:
            if wandb.config.diag_freeze:
                wandb_name = f'EHR-Transformer_{str(seed)}_diag_freeze_SWEEPS'
                tags = (str(seed), 'diag_freeze', wandb.config.loss_type)
            else:
                wandb_name = f'EHR-Transformer_{str(seed)}_SWEEPS'
                tags = (str(seed), wandb.config.loss_type)

        name = f'{wandb_name}_{str(date_dir)}-{str(time.strftime("%H:%M:%S"))}' 
        log_name = f'{name}.log'
        writer.name = f'{wandb_name}_{str(date_dir)}-{str(time.strftime("%H:%M:%S"))}'
        ...
        args_dict = {
            ...
            'attn_dropout': wandb.config.dropout_rate,
...
if __name__ == '__main__':
    sweep_configuration = {
        ...
        'parameters': {
            ...
            'mask_prob': {'values': [0.25, 0.3]},
            'loss_type': {'values': ['bce', 'balanced_bce', 'focalloss']},
            'pretrained_type': {'values': ['te3-small', 'te3-large', 'te-ada002']},
            'diag_freeze': {'values': [True, False]},
            'gamma': {'values': [1.0, 2.0, 3.0]},
            'alpha': {'values': [0.25, 0.5]},
            'mlm_lambda': {'values': [0.2, 0.3, 0.4]}
        }
    }
